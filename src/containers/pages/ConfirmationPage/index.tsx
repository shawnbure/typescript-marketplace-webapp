...
import { useStakeCollectionMutation } from "services/collections";

import { GetTransactionRequestHttpURL, GetTokenRequestHttpURL } from "utils";
import {
    ACCEPT_OFFER,
    BUY,
    LIST,
...
export const ConfirmationPage = () => {
    ...
    const [transactionHash] = useState(
        getQuerystringValue(queryString, "txHash") || ""
    );
    ...
    useEffect(() => {
        switch (action.toUpperCase()) {
            ...
            case LIST:
                ...
                setPriceNominal(
                    getQuerystringValue(queryString, "price") || ""
                );
                break;
            case STAKE:
                txFailed == false
            ...
            case MINT:
                ...
                setNumberMinted(
                    Number(getQuerystringValue(queryString, "number_minted")) ||
                        0
                );
                setNftLink(
                    window.location.origin +
                        routePaths.collection.replace(
                            ":collectionId",
                            collectionId
                        )
                );
            ...
            case START_AUCTION:
                ...
                setStartDate(
                    Number(getQuerystringValue(queryString, "start_date")) || 0
                );
                setEndDate(
                    Number(getQuerystringValue(queryString, "end_date")) || 0
                );
                break;
            case END_AUCTION:
                txFailed == false
            ...
            if (httpRequest.readyState == 4 && httpRequest.status == 200) {
                if (httpRequest.responseText) {
                    ...
                    try {
                        ...
                        setNftLink(
                            `${window.location.origin}/token/${jsonResponse.collection}/${jsonResponse.nonce}`
                        );
    ...
    const setDatabaseRecord = async () => {
        ...
        switch (action.toUpperCase()) {
            case BUY:
                response = buyTokenFromClientTrigger({
                    payload: formattedData,
                });
                break;
            case LIST:
                response = listTokenFromClientTrigger({
                    payload: formattedData,
                });
                break;
            case STAKE:
            case UNSTAKE:
                response = stakeTokenFromClientTrigger({
                    payload: formattedData,
                });
                break;
            case STAKECOL:
            case UNSTAKECOL:
                ...
                response = stakeCollectionFromClientTrigger({
        ...
        if (isTransactionSuccessful) {
            toast.success(
                ENG_TX_SUCCESS_MESSAGE.toString().replace(
                    "{{tokenName}}",
                    tokenName
                ),
                {
                                    ...
                                    <div className="u-margin-top-spacing-3 u-margin-bottom-spacing-5 u-text-small justify-center">
                                        {isTransactionSuccessful ? (
                                            <div className="u-margin-top-spacing-3 u-margin-bottom-spacing-5 u-text-small justify-center">
                                                ...
                                                >
                                                    <a
                                                        href={`https://twitter.com/intent/tweet?text=${nftLink}`}
                                                        target="_new"
                                                    >
                                                        <FontAwesomeIcon
                                                            style={{
                                                            ...
                                                            color="#1DA1F2"
                                                            title={`${ENG_SHARE_TO} Twitter`}
                                                        />
                                                    </a>
                                                    <a
                                                        href={`https://api.whatsapp.com/send?text=${nftLink}`}
                                                        target="_new"
                                                    >
                                                        <FontAwesomeIcon
                                                            style={{
                                                            ...
                                                            color="#25D366"
                                                            title={`${ENG_SHARE_TO} WhatsApp`}
                                                        />
                                                    </a>
                                                    <a
                                                        href={`https://www.facebook.com/sharer/sharer.php?u=#${nftLink}`}
                                                        target="_new"
                                                    >
                                                        <FontAwesomeIcon
                                                            style={{
                                                            ...
                                                            color="#4267B2"
                                                            title={`${ENG_SHARE_TO} Facebook`}
                                                        />
                                                    </a>
                                                    <a
                                                        href={`https://t.me/share/url?url=${nftLink}`}
                                                        target="_new"
                                                    >
                                                        <FontAwesomeIcon
                                                            style={{
                                                    ...
                                                    <FontAwesomeIcon
                                                        ...
                                                        size="3x"
                                                        icon={faIcons.faCopy}
                                                        title={`${ENG_COPY_TO_CLIPBOARD_TITLE}`}
                                                    />
                                                </div>
                                            </div>
                                        ) : null}
                                        <p>
                                            ...
                                            >
                                                <FontAwesomeIcon
                                                    ...
                                                    className="c-navbar_icon-link u-text-theme-blue-anchor "
                                                    icon={
