...
export const TokenPage: (props: any) => any = ({}) => {
  ...
  const { data: tokenData } = walletAddressParam
    ? gatewayTokenData
    : tokenResponseData;
  ...
  const getBaseTokenData = (tokenData: any) => {
    ...
    const imageLink: string = isOurs
      ? token.imageLink
      : atob(token?.uris?.[0] || "");
    const metadataLink: string = isOurs
      ? token.metadataLink
      : atob(token?.uris?.[1] || "");
  ...
  if (!isOurs && tokenData.tokenData.balance === "0" && walletAddressParam) {
    return (
      <>
        ...
        <p className="my-10 text-2xl text-center">
          is not the owner of {collectionId} {tokenNonce}
        </p>
      </>
    );
  }
  ...
  const tokenListings = getTokenTransactionsData?.data?.filter(
    (transaction: any) =>
  ...
  const mapActivityTableData = getTokenTransactionsData?.data?.map(
    (transaction: any, index: number) => {
