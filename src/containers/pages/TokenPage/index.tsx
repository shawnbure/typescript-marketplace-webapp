...
export const TokenPage: (props: any) => any = ({}) => {
    ...
    const getInitialTxs = async () => {
        ...
        if (response?.error) {
            if (
                (response?.error.data.error as string).indexOf(
                    "record not found"
                ) !== -1
            ) {
                return;
    ...
    const isCurrentTokenOwner: boolean =
    ...
    const priceTokenDollarsFixed = parseFloat(`${priceTokenDollars}`).toFixed(
        3
    );
        ...
        (transaction: any, index: number) => {
            ...
            const event = (
                <>
                    <FontAwesomeIcon
                        width={"20px"}
                        icon={mapTypeEvents[type].icon}
                    />
                    <span className="inline-block ml-4">
                        {mapTypeEvents[type].title}
                    </span>
                </>
            );
    ...
    const mapBidsTableData = tokenBidsData?.data?.map(
        (offerData: any, index: number) => {
            ...
            const isCurrentBidder: boolean =
                bidderAddress === userWalletAddress;
                            ...
                            <div className="p-token-page_token-data u-border-radius-2 u-overflow-hidden">
                                ...
                                >
                                    ...
                                    <div className="accordion_content c-property_rarity_message">
                                        Rarity Trait % Calculated Only for Items
                                        Listed on Youbei
                                    </div>
                                </Collapsible>
