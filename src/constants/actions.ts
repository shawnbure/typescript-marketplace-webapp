export const BUY: string = "BUY";
export const MINT: string = "MINT";
export const LIST: string = "LIST";
export const SELL: string = "SELL";
export const LOGIN: string = "LOGIN";
export const STAKE: string = "STAKE";
export const UNSTAKE: string = "UNSTAKE";
export const STAKECOL: string = "STAKECOL";
export const UNSTAKECOL: string = "UNSTAKECOL";
export const AUCTION: string = "AUCTION";
export const MAKE_BID: string = "MAKE_BID";
export const WITHDRAW: string = "WITHDRAW";
export const MAKE_OFFER: string = "MAKE_OFFER";
export const START_AUCTION: string = "START_AUCTION";
export const END_AUCTION: string = "END_AUCTION";
export const ACCEPT_OFFER: string = "ACCEPT_OFFER";
export const CANCEL_OFFER: string = "CANCEL_OFFER";


